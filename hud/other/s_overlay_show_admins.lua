...
function showStaff( thePlayer, commandName )
	...
	if(logged==1) then
		...
		for k, arrayPlayer in ipairs(players) do
			...
			if (logged == 1) and not dvpUsernames[username] then
				if tonumber(getElementData( arrayPlayer, "admin_level" )) < 10 then
					if exports.global:isStaffOnDuty(arrayPlayer) then
		...
		for k, v in ipairs(admins) do
			...
			local hiddenAdmin = getElementData(arrayPlayer, "hiddenadmin")
			if exports.global:isStaffOnDuty(arrayPlayer) then
				if hiddenAdmin == 0 and exports.integration:isPlayerTrialAdmin(thePlayer) then
	...
	if(logged==1) then
		...
		for k, arrayPlayer in ipairs(players) do
			local logged = getElementData(arrayPlayer, "loggedin")
			if logged == 1 then
				if exports.integration:isPlayerSupporter(arrayPlayer) and exports.global:isStaffOnDuty(arrayPlayer) then
			...
			if logged == 1 then
				if exports.integration:isPlayerVCTMember(arrayPlayer) then
					...
					if (hiddenAdmin == 1) then
						stuffToPrint = "-    "..(exports.integration:isPlayerVehicleConsultant(arrayPlayer) and "Lider" or "Üye").." (Gizli) "..exports.global:getPlayerName(arrayPlayer).." ("..getElementData(arrayPlayer, "account:username")..")"
					else
						stuffToPrint = "-    "..(exports.integration:isPlayerVehicleConsultant(arrayPlayer) and "Lider" or "Üye").." "..exports.global:getPlayerName(arrayPlayer).." ("..getElementData(arrayPlayer, "account:username")..")"
					end
		...
		for k, arrayPlayer in ipairs(players) do
			...
			if logged == 1 then
				if exports.integration:isPlayerScripter(arrayPlayer) then
					...
					local stuffToPrint
					if (hiddenAdmin == 1) then
						stuffToPrint = "-    (Gizli) "..tostring(adminTitle).." "..exports.global:getPlayerName(arrayPlayer).." ("..getElementData(arrayPlayer, "account:username")..")"
					else
