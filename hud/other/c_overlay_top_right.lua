...
local awfont = exports.fonts:getFont("FontAwesome", 10)
local BizNoteFont20 = exports.fonts:getFont("AwesomeFont", 16)
...
setTimer(function ()
	if show and not getElementData(localPlayer, "integration:previewPMShowing") and getElementData(localPlayer, 'loggedin') == 1 then 
		if ( getPedWeapon( localPlayer ) ~= 43 or not getPedControlState( localPlayer, "aim_weapon" ) ) then
			...
			roundedRectangle(sx-toBeDrawnWidth-5+posXOffset, 5+posYOffset, toBeDrawnWidth, 16*(#content)+35, tocolor(210,210,210,210), tocolor(210,210,210,210))

			--dxDrawImage ( sx-5+posXOffset-32, (5+posYOffset)+5, 30, 30, (pinned and "pinned.png" or "pin.png" ),0, 0, 0, tocolor(0,0,0), false )
			for i=1, #content do
				if content[i] then
					...
					if i == 1 or content[i][7] == "title" then --Title
						...
						dxDrawText( content[i][1] or "" , sx-toBeDrawnWidth+10+posXOffset, (16*i)+posYOffset, toBeDrawnWidth-5, 15, tocolor ( 15, 15, 15), 1, BizNoteFont20 )
					else
						dxDrawText( content[i][1] or "" , sx-toBeDrawnWidth+10+posXOffset, (16*i)+posYOffset, toBeDrawnWidth-5, 15, tocolor ( content[i][2] or 15, content[i][3] or 15, content[i][4] or 15, content[i][5] or 255 ), content[i][6] or 1, Roboto )
					end
				end
			end
		end
