...
local notificationFont = exports.fonts:getFont("Roboto", 14)
local notificationFontBold = exports.fonts:getFont("AwesomeFont", 15)
...
local types = {}
local details = {
    ["warning"] = {" Uyarı", "warning", "e.mp3"},
    ["error"] = {" Hata", "error", "e.mp3"},
    ["info"] = {" Bilgi", "information", "e.mp3"},
    ["mod"] = {" Duyuru", "information", "e.mp3"},
    ["models"] = {" Modeller yükleniyor..", "information", "e.mp3"},
    ["success"] = {" Başarılı", "success", "e.mp3"},
    ["aduty"] = {" Yetkili Duyurusu", "adminduty", "e.mp3"},
    ["buy"] = {" Finansal Güncelleme", "buy", "e.mp3"},
    ["twitter"] = {" Twitter", "twitter", "e.mp3"},
    ["pet"] = {" Hayvan Kontrolü", "pet", "e.mp3"},
}
...
function drawnBoxes()
    _sx, _sy = sx, sy
    local enabled, sx, nowY, w,h,sizable,turnable, sizeDetails, t, columns = true, 25, 360, 320, 45, 1, 1
    _nowY = nowY
    ...
    for k,v in ipairs(cache) do
        ...
        local type = v["type"]
        local tick = v["tick"]
        local showtime = v["showtime"] or 6000
        ...
        if state == "fadeIn" then
            ...
            if progress < 1 then
                pos = {interpolateBetween(sx - 90, nowY, 0, sx, nowY, 0, progress, 'OutQuad')}
        ...
        elseif state == "fadeOut" then
            ...
            alpha = {interpolateBetween(190, 255,0, 0,0,0, progress, 'OutQuad')}
        ...
        dxDrawRectangle(pos[1], pos[2], boxWidth, 50, tocolor(40, 40, 40, alpha[1]),true)
        dxDrawText(msg, pos[1] + 8 + 10, pos[2] + 26, 0, 0, tocolor(250, 250, 250, alpha[1]), 0.78, notificationFont, 'left', 'top',false,false,true,true,false)
        nowY = nowY + between
        dxDrawText(details[type][1], pos[1] + 8 + 10, pos[2] + 5, 0, 0, tocolor(250, 250, 250, alpha[2]), 0.75, notificationFontBold, 'left', 'top',false,false,true,false,false)
